...
def get_digital_input(io_num):\n\
  ...
  if io_num > 16:\n\
    ...
    return -1\n\
\n\
  m_pipe = subprocess.Popen(\"rostopic echo -n 1 probot_status | grep -w inputIOs\", shell=True, stdout=subprocess.PIPE)\n\
  m_pipe_out = m_pipe.stdout.readlines()\n\
  # line format:\n\
  # inputIOs: [False, True, False, False, False, False, False, False, False, False, False, False, False, False, False, False]\n\
  for line in m_pipe_out:\n\
    input_val_str = line.replace(\"False\", \"0\").replace(\"True\", \"1\").strip()\n\
...
Blockly.Python['set_digital_output'] = function(block) {
  ...
  var code = "";
  code += "\nio_type   = TYPE_DIGITAL";
  code += "\nio_num    = " + io_num.toString();
  code += "\nio_status = " + io_level.toString();
  code += "\n\
if probot_initialized != 1:\n\
  rospy.logerr(\"Run program failed! Robot is not initialized! Please place the block 机械臂设置--机械臂初始化 at the beginning.\")\n\
  sys.exit()\n\
if control_mode != CONTROLLER_CTRL_LIVE:\n\
  ...
  sys.exit()\n\
if io_num > 24:\n\
  rospy.logerr(\"Run program failed! The IO number specified is out of range(1-24).\")\n\
  sys.exit()\n\
if io_num > 16:\n\
  io_type = TYPE_RELAY\n\
  io_num = io_num - 16\n\
...
if io_status != 0:\n\
  io_status = io_mask\n\
shell_cmd = \"rostopic pub -1 probot_set_output_io probot_msgs/SetOutputIO -- '{type: \" + str(io_type) + \", mask: \" + str(io_mask) + \", status: \" + str(io_status) + \"}' >/dev/null 2>&1\"\n\
os.system(shell_cmd)\n\
";
  return code;
};
